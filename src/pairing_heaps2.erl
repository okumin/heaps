...
out({Item, []})   -> {{value, Item}, empty};
out({Item, Heap}) ->
    Pairs = make_pairs(Heap, []),
    H = lists:foldl(fun(X, Acc) -> merge(Acc, X) end, empty, Pairs),
    {{value, Item}, H}.
...
%% Internal Functions
%%----------------------------------------------------------------------------------------------------------------------
-spec make_pairs([heap(Item)], [heap(Item)]) -> heap(Item).
make_pairs([], Acc)            -> Acc;
make_pairs([H], Acc)           -> [H | Acc];
make_pairs([H1, H2 | Hs], Acc) -> make_pairs(Hs, [merge(H1, H2) | Acc]).
